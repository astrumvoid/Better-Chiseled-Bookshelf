...
import perfectvoid.enchantlib.config.Configs.obstructionType;
...
public class EnchantmentPowerUtil {
    ...
    public static boolean canBlockPath(World world, BlockPos pos, obstructionType obType) {
        if (enableAllowList && allowList.contains(Identifier.of(world.getBlockState(pos).getRegistryEntry().getIdAsString()))) return false;
        if (enableDenyList && denyList.contains(Identifier.of(world.getBlockState(pos).getRegistryEntry().getIdAsString()))) return true;
    ...
    public static List<BlockPos> getPowerProvidersInArea(World world, BlockPos origin, List<BlockPos> areaToSearch, obstructionType obType, boolean getMoreShelves) {
        ...
        for (BlockPos maxSearchPos : areaToSearch) {
            BlockPos.Mutable searchOffset = originArea.stream().sorted(Comparator.comparing(maxSearchPos::getManhattanDistance)).toList().getFirst().mutableCopy();
    ...
    public static boolean isPathObstructed(World world, BlockPos origin, BlockPos.Mutable providerOffset, obstructionType type) {
        ...
        BlockPos.Mutable searchPos = new BlockPos.Mutable();
        BlockPos closestBlockToOrigin = originArea.stream().sorted(Comparator.comparing(providerOffset::getManhattanDistance)).toList().getFirst();
